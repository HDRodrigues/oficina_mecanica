...
RSpec.describe Quotes::RejectQuoteByToken do
  ...
  let(:reject_quote) do
    double("RejectQuote").tap do |uc|
      allow(uc).to receive(:call).with(id: 1).and_return(Shared::Result.success(quote))
    end
  end

  let(:use_case) do
    described_class.new(quote_repository: quote_repository, reject_quote: reject_quote)
  end

  describe "#call" do
    it "resolves the quote by token and delegates to RejectQuote" do
      result = use_case.call(approval_token: "tok-123")

      expect(result).to be_success
      expect(reject_quote).to have_received(:call).with(id: 1)
    end
    ...
    it "returns failure without delegating when the token is unknown" do
      ...
      expect(reject_quote).not_to have_received(:call)
    end

    it "propagates a failure from RejectQuote" do
      allow(reject_quote).to receive(:call).and_return(Shared::Result.failure("Failed to reject work order"))

      result = use_case.call(approval_token: "tok-123")

      expect(result).to be_failure
      expect(result.error).to eq("Failed to reject work order")
    end
  end
end
